...
function ParkingSignedDist(x0,xF,N,Ts,L,ego,XYbounds,nOb,vOb, A, b,fixTime,xWS,uWS)
	...
	m = Model(solver=IpoptSolver(hessian_approximation="exact",mumps_pivtol=1e-6,alpha_for_y="min",recalc_y="yes",
	                             mumps_mem_percent=6000,max_iter=600,tol=1e-5, print_level=0,
	...
	##############################
	#input constraints
	@constraint(m, [i=1:N], -0.45 <= u[1,i] <= 0.45) 	# steering
	@constraint(m, [i=1:N], -1 <= u[2,i] <= 1)	# accelartion

	#state constraints
	@constraint(m, [i=1:N+1], XYbounds[1] <= x[1,i] <= XYbounds[2])
	@constraint(m, [i=1:N+1], XYbounds[3] <= x[2,i] <= XYbounds[4])

	@constraint(m, [i=1:N+1], -2  <= x[4,i] <= 3)	
	...
	if fixTime == 1
		for i in 1:N
			if i==1
				...
				#@constraint(m,-0.5<=(u0[2]-u[2,i])/Ts <= 0.5)
			else
				@constraint(m,-0.6<=(u[1,i-1]-u[1,i])/Ts <= 0.6)
				#@constraint(m,-0.5<=(u[2,i-1]-u[2,i])/Ts <= 0.5)
			end
		end
	else
		for i in 1:N
			if i==1
				...
				#@NLconstraint(m,-0.5<=(u0[2]-u[2,i])/(timeScale[i]*Ts) <= 0.5)
			else
				@NLconstraint(m,-0.6<=(u[1,i-1]-u[1,i])/(timeScale[i]*Ts) <= 0.6)
				#@NLconstraint(m,-0.5<=(u[2,i-1]-u[2,i])/(timeScale[i]*Ts) <= 0.5)
			end
		end
	end
