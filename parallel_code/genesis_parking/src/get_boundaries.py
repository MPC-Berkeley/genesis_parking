...
def main():
    ...
    offsetY				= -offset*sin(yawRef)
    GM_ref[1]			= GM_ref[1] + 180.0/pi*offsetX/(6371000.0*cos(pi*GM_ref[0]/180.0))
    GM_ref[0]			= GM_ref[0] + 180.0/pi*offsetY/6371000.0
    
    # calculate difference using same reference point from google map and dGPS
    # apply this "diff" to all coordinates from google map 
    ...
    # calculate final position based on the center of the spot
    estimate			= -1.3
    xF					= estimate*cos(yawF)
    yF					= estimate*sin(yawF)

    # convert initial position to rear axle
    x0					= x0 - (Lr)*cos(yaw0)
    y0					= y0 - (Lr)*sin(yaw0)

    # formulate initial and final states
    s0                  = array( [x0, y0, yaw0, 0] )
    sF                  = array( [xF, yF, yawF, 0] )
    sF					= sF - [spotBuffer * cos(sF[2]), spotBuffer * sin(sF[2]), 0, 0] # adjust the final state
    
    gps_coords			= array( [GM_gc_lat, GM_gc_lng] ) # save the coordinate reference for later use
    ...
    b_fl = tuple(map(operator.add, b_fl, (-spot_margin*cos(np.pi/2-yawRef), spot_margin*sin(np.pi/2-yawRef)))) # widen the spot if necessary, currently not in use
    
    b_fr = coord_sys.get_XY(gps_fr[0], gps_fr[1]) # XY coord, front right
    b_fr = tuple(map(operator.add, b_fr, (spot_margin*cos(np.pi/2-yawRef), -spot_margin*sin(np.pi/2-yawRef))))
    
    b_rl = coord_sys.get_XY(gps_rl[0], gps_rl[1]) # XY coord, rear left
    b_rl = tuple(map(operator.add, b_rl, (-spot_margin*cos(np.pi/2-yawRef), spot_margin*sin(np.pi/2-yawRef))))
    
    b_rr = coord_sys.get_XY(gps_rr[0], gps_rr[1]) # XY coord, rear right
    b_rr = tuple(map(operator.add, b_rr, (spot_margin*cos(np.pi/2-yawRef), -spot_margin*sin(np.pi/2-yawRef))))
