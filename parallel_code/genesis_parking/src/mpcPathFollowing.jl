...
type MpcModel
	...
	# changed steering input from [-0.5,0.5] to [-0.4,0.4]
		...
		# delta-U cost
		W_steering = 100.0
		W_acceleration = 100.0
		@NLexpression(mdl, costDU, W_steering * ( (uOL[1,1]-u0[1])^2 +  sum( (uOL[1,i+1] - uOL[1,i])^2 for i = 1:N-1 )) + W_acceleration * ( (uOL[2,1]-u0[2])^2 + sum( (uOL[2,i+1] - uOL[2,i])^2  for i=1:N-1)))
