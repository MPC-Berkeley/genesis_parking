...
Ts 			= 0.6/3*sampleN 				############################################### test debugging
...
Lf_bumber	= planning_data["length_front_bumber"]
Lr_bumber	= planning_data["length_rear_bumber"]
...
for i = 1 : lengthPath
	xRearLeft	= sp10[1,i] - Lr_buffer*cos(sp10[3,i]) - (wl + side_buffer) * sin(sp10[3,i])
	yRearLeft	= sp10[2,i] - Lr_buffer*sin(sp10[3,i]) + (wl + side_buffer) * cos(sp10[3,i])
	
	xRearRight	= xRearLeft + (wl + wr + 2 * side_buffer) * sin(sp10[3,i])
	yRearRight	= yRearLeft - (wl + wr + 2 * side_buffer) * cos(sp10[3,i])
	
	xFrontLeft	= xRearLeft + (Lr_bumber+Lf_bumber) * cos(sp10[3,i])
	yFrontLeft	= yRearLeft + (Lr_bumber+Lf_bumber) * sin(sp10[3,i])
	
	xFrontRight	= xFrontLeft + (wl + wr + 2 * side_buffer) * sin(sp10[3,i])
	yFrontRight	= yFrontLeft - (wl + wr + 2 * side_buffer) * cos(sp10[3,i])
...
# plotTraj(sp10',up10',size(sp10, 2)-1,ego,L,nOb,vOb,lOb,"Trajectory generated by H-OBCA",1)
