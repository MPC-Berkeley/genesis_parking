...
const VEHICLE_RADIUS = 0.945 #[m]; radius of rear ball; 0.945
const BUBBLE_DIST = 3.109 #[m]; distance to "forward bubble"; 3.109

##### Fast Comp Time values from Alex Liniger ######
const OB_MAP_RESOLUTION = 0.1 #[m]; obstacle resolution 0.1
const YAW_GRID_RESOLUTION = deg2rad(1.0) #[m]; 10.0 /// 1.0
const N_STEER = 10.0 # number of steer command; 10.0 seems OK /// 10
## For Backwards Parking
const XY_GRID_RESOLUTION = 0.1 #[m];//0.3 0.1
const MOTION_RESOLUTION = 0.1 #[m];0.1
###################################################

const USE_HOLONOMIC_WITH_OBSTACLE_HEURISTIC = true
const USE_NONHOLONOMIC_WITHOUT_OBSTACLE_HEURISTIC = false

const SB_COST = 100.0 # switch back penalty cost 10
const BACK_COST = 0.0 # backward penalty cost 0.0
const STEER_CHANGE_COST = 10.0 # steer angle change penalty cost
const STEER_COST = 0.0  # steer angle  penalty cost
const H_COST = 1.0 # Heuristic cost; higher -> heuristic; 1.0
 
const WB = 2.7 #[m]; 7.0
# const MAX_STEER = 0.6  #[rad]
const MAX_STEER = 0.45	# [rad] new/updated
...
function calc_motion_inputs()
    ...
    # d = vcat([1.0 for i in 1:length(u)])  # just positive speed  
    u = vcat(u,u)
