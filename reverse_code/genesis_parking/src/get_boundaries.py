...
def main():
    ...
    xF					= -(Lr+offset)*cos(yawF)+(length_front_bumber+Lr-length_rear_bumber+Lr-0.5)*cos(yawF)
    yF					= -(Lr+offset)*sin(yawF)+(length_front_bumber+Lr-length_rear_bumber+Lr-0.5)*sin(yawF)
    ...
    b_fl = tuple(map(operator.add, b_fl, (-spot_margin*cos(np.pi/2-yawF), spot_margin*sin(np.pi/2-yawF)))) # widen the spot if necessary, currently not in use
