...
class State():
	...
	def compute_xy(self):
		...
		self.x 		= R_earth * delta_lon * cos(lat_avg)
		self.y 		= R_earth * delta_lat
...
def pub_loop():
	...
	dgps_offset = rospy.get_param("/dgps_global_offset")	# not in use
	axle_dist   = rospy.get_param("/gps_to_rear_axle_dist")	# center of mass to rear axle
